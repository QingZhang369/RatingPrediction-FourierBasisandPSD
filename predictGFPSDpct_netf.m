%%研究预测准确率是否依赖于评分表中已知元素所占的百分比,对比RKHS+kBR,RKHS+PSD,CF
clear all;
close all;
...
if implement
    ...
    for r=1:10
        ...
        for pct=0.01:0.005:0.05              %%已经标签所占的百分比体现在这
            ntest=lbs-fix(Un*In*pct);         %%测试个数=总评分数个数-已有标签的个数(总数据分成两部分，一部分参与实验(非零)，一部分用于测试设置为0)
            [utest,s]=GetTestSet1(os,ntest);  %%输入初始评分表和测试集中评分个数；返回测试集和训练集s(被用作测试的打分不用于训练，在评分表中将对应项置0)
 ...
 %%other methods
            ...
            [G,K,as,mr,mrc]=GKas(s0,mask,gama,true);  %%此处as是AdjustUI(s)得到的;

            %% RKHS+kBR (Yang-2020) 
            f=KBreconstucter(G,as,mask,K,alpha,beta,10);  %%wf不显示时，即是模型一(不考虑psd的模型)
            f=f+mr+mrc;
            ff(rs,cs)=f;
            idx=find(ff>5); %if a rating is larger than 5 then set it to 5
            ff(idx)=5;
            ...
            wf=p(psd);
            wf=G.lmax*wf/max(wf);
            f=KBreconstucter(G,as,mask,K,alpha,beta,10,wf); %psd模型
            f=f+mr+mrc; 
            ff(rs,cs)=f;
            idx=find(ff>5); %if a rating is larger than 5 then set it to 5
            ff(idx)=5;
...
%MAE基于用户(user-first,item-first)和基于项目(user-first,item-first)的分开画图

figure(1)
plot(1:9,mean(netfpct_us3(6:10,:)),'o-','LineWidth',2,'MarkerSize',8);%去掉颜色字母，则会随机分配颜色
hold on
...
%RMSE基于用户(user-first,item-first)和基于项目(user-first,item-first)的分开画图

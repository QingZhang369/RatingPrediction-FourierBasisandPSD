%%netf数据集,对比GFPSD和RKHS+kBR和RKHS+PSD（copy code of predictKB.m,predictmyKBPSD.m），result from workstation
clear all;
close all;
...
if implement
   load('netfset.mat');
    ...
    mae_us=zeros(30,1);   %前10个是RKHS+kBR(前5个是项目优先，后5个是用户优先)，中间10个是RKHS+PSD,最后10个是GFPSD
    rmse_us=zeros(30,1);
    mae_it=zeros(30,1);
    rmse_it=zeros(30,1);
    mtu=zeros(30,1);
...
%            
        ...
        items=unique(utest(:,2));  %utest的第二列是用户对于评分的项目，取不重复的且按递增排序 
        k=5;
        ...
        Uk=U(:,1:k);
        for j=1:numel(items)
            labels=find(mask1(:,items(j))); %寻找第items(j)列不为零的位置
        ...
        f=f+mr+mrc;             %%？？？为什么+mr+mrc，见AdjustUI.m，调整后的评分表aUI=(UI-repmat(mU+mUI,1,I)).*nonEl;
%         f=f+mu;
        ff(rs,cs)=f;
...
disp(['GR+PSD  ',num2str(mean(GFPSDvskBRPSDnetf(16:20,1))),'+-',num2str(std(GFPSDvskBRPSDnetf(16:20,1))),'     ',num2str(mean(GFPSDvskBRPSDnetf(16:20,2))),'+-', ...
...
disp(['GFPSD+appro   ',num2str(mean(GFPSDvskBRPSDnetf(26:30,1))),'+-',num2str(std(GFPSDvskBRPSDnetf(26:30,1))),'     ',num2str(mean(GFPSDvskBRPSDnetf(26:30,2))),'+-', ...
...
disp(['GR+PSD   ',num2str(mean(GFPSDvskBRPSDnetf(16:20,4))),'+-',num2str(std(GFPSDvskBRPSDnetf(16:20,4))),'     ',num2str(mean(GFPSDvskBRPSDnetf(16:20,5))),'+-', ...
...
disp(['GFPSD+appro   ',num2str(mean(GFPSDvskBRPSDnetf(26:30,4))),'+-',num2str(std(GFPSDvskBRPSDnetf(26:30,4))),'     ',num2str(mean(GFPSDvskBRPSDnetf(26:30,5))),'+-', ...
